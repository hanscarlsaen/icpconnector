...
export function loadAgentConfig(agentId: string): AgentConfig {
  ...
  const chatIds = (raw['chat_ids'] as string[]) ?? undefined;
  const systemPrompt = (raw['system_prompt'] as string) ?? undefined;
  const skills = (raw['skills'] as string[]) ?? undefined;
  const skillEnvRaw = (raw['skill_env'] as Record<string, string>) ?? undefined;

  // Resolve skill_env: values that match a key in .env are treated as env var references.
  // Values that look like relative file paths are resolved against the agent directory.
  let skillEnv: Record<string, string> | undefined;
  if (skillEnvRaw) {
    const allEnv = readEnvFile(Object.values(skillEnvRaw));
    skillEnv = {};
    for (const [key, value] of Object.entries(skillEnvRaw)) {
      // If the value is an env var name that exists, resolve it (runtime env wins over .env file)
      const resolved = process.env[value] ?? allEnv[value];
      if (resolved) {
        skillEnv[key] = resolved;
        continue;
      }
      // Try as a relative file path under the agent directory
      const asPath = path.join(agentDir, value);
  ...
  return { name, description, botTokenEnv, botToken, model, chatIds, systemPrompt, skills, skillEnv, obsidian };
}

...
if (AGENT_ID !== 'main') {
  ...
  // Inject skill environment variables into process.env
  if (agentConfig.skillEnv) {
    for (const [key, value] of Object.entries(agentConfig.skillEnv)) {
      process.env[key] = value;
    }
    logger.info({ agentId: AGENT_ID, keys: Object.keys(agentConfig.skillEnv) }, 'Injected skill_env');
  }

  // Inject PROJECT_ROOT so Claude Code subprocess and run-skill.sh can locate the repo
  process.env.PROJECT_ROOT = PROJECT_ROOT;

  // Auto-create skill symlinks so Claude Code discovers SKILL.md files
  // from the agent's CWD via settingSources: ['project']
  if (agentConfig.skills && agentConfig.skills.length > 0) {
    for (const skillName of agentConfig.skills) {
      const skillMdSource = path.join(PROJECT_ROOT, 'skills', skillName, 'SKILL.md');
      if (!fs.existsSync(skillMdSource)) {
        logger.warn({ agentId: AGENT_ID, skill: skillName }, 'SKILL.md not found, skipping symlink');
        continue;
      }
      try {
        const skillTargetDir = path.join(agentDir, '.claude', 'skills', skillName);
        const skillMdLink = path.join(skillTargetDir, 'SKILL.md');
        fs.mkdirSync(skillTargetDir, { recursive: true });
        try { fs.unlinkSync(skillMdLink); } catch { /* doesn't exist */ }
        fs.symlinkSync(skillMdSource, skillMdLink);
      } catch (err) {
        logger.warn({ agentId: AGENT_ID, skill: skillName, err }, 'Failed to create skill symlink');
      }
    }
    logger.info({ agentId: AGENT_ID, skills: agentConfig.skills }, 'Created skill symlinks');
  }

  // Use agent.yaml system_prompt, fall back to CLAUDE.md file
  let systemPrompt: string | undefined = agentConfig.systemPrompt;
  if (!systemPrompt && claudeMdPath) {
    try {
